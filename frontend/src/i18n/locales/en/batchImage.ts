export default {
  batchImage: {
    ...
    itemResult: {
      ...
      emptyImageOutput: 'The upstream returned a result, but this item has no image content. This usually means a single upstream generation failed or was blocked by safety policies.',
    ...
    create: {
      ...
      selectKeyPlaceholder: 'Select an OpenAI or Gemini API key',
      noKeysHint: 'No OpenAI or Gemini API key is available for batch image generation. Create one and bind it to an OpenAI or Gemini group with batch image generation enabled first.',
      model: 'Model',
      imageSize: 'Image size',
      imageSizeHint: 'This model uses a fixed 1K image size in the batch UI.',
      aspectRatio: 'Aspect ratio / pixels',
      openaiImageSizeHint: 'Available billing tiers depend on group permissions and configured prices. The longest edge determines the tier, not an exact pixel size.',
      openaiDimensionsHint: 'GPT Image 2: edges must be multiples of 16, at most 3840 px; 655,360–8,294,400 total pixels; aspect ratio at most 3:1. Widescreen requires 2K or 4K. The 4K square preset is 2880×2880.',
    ...
    guide: {
      ...
      step1: '1. Select an OpenAI or Gemini API key with batch image generation enabled. The model list shows the models available to that key’s group.',
    ...
    messages: {
      ...
      selectApiKey: 'Select an available OpenAI or Gemini API key.',
      ...
      noCompatibleAccount: 'No usable upstream batch image account is available for this key’s group. Contact an administrator to check the group’s schedulable OpenAI or Gemini API key or Vertex service account and model support.',
