export default {
  batchImage: {
    ...
    itemResult: {
      ...
      cancelled: '任务已取消',
      waiting: '等待生成结果',
      emptyImageOutput: '上游返回了结果，但这条没有图片内容。通常是上游单条生成失败或被安全策略拦截。',
    ...
    create: {
      ...
      selectKeyPlaceholder: '请选择 OpenAI 或 Gemini API Key',
      noKeysHint: '当前没有可用于批量生图的 OpenAI 或 Gemini API Key。请先创建并绑定已开启批量生图的 OpenAI 或 Gemini 分组。',
      model: '模型',
      imageSize: '图片尺寸',
      imageSizeHint: '此模型在批量任务界面固定按 1K 图片提交。',
      aspectRatio: '宽高比 / 像素尺寸',
      openaiImageSizeHint: '可用计费档位取决于分组权限和已配置价格。按最长边划分档位，档位名称不代表固定像素尺寸。',
      openaiDimensionsHint: 'GPT Image 2：两边均为 16 的倍数且不超过 3840 像素，总像素 655,360–8,294,400，长宽比不超过 3:1。宽屏需选择 2K 或 4K；4K 正方形预设为 2880×2880。',
      outputFormat: '输出格式',
    ...
    guide: {
      title: '批量生图使用说明',
      uiTitle: '当前界面如何使用',
      step1: '1. 选择已开启批量生图的 OpenAI 或 Gemini API Key，模型列表会按该 Key 所属分组可用模型展示。',
    ...
    messages: {
      ...
      selectApiKey: '请选择可用的 OpenAI 或 Gemini API Key',
      ...
      noCompatibleAccount: '当前密钥所属分组没有可用的批量生图上游账号。请联系管理员检查：该分组是否绑定了可调度的 OpenAI 或 Gemini API Key 或 Vertex 服务账号，以及账号是否支持所选模型。',
