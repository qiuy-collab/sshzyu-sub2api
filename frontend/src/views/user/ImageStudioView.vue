        ...
        <template v-else>
          ...
          <div class="studio-world" :style="worldStyle">
            ...
            <article v-for="node in board.nodes" :key="node.id" :data-node-id="node.id" :data-node-type="node.type"
              class="studio-node" :class="[`studio-node-${node.type}`, { 'is-selected': selectedId === node.id, 'is-source': connectSource === node.id }]"
            ...
            >
              <div class="studio-node-header" @pointerdown.stop="startNodeDrag($event, node)">
                ...
                <input v-model="node.title" maxlength="200" :aria-label="t('imageStudio.nodeTitle')" @pointerdown.stop @click.stop @focus="remember" />
                <button class="studio-grip" :aria-label="t('imageStudio.moveNode')" :title="t('imageStudio.moveNode')" tabindex="-1">⠿</button>
              </div>
              <template v-if="node.type === 'text'">
                <textarea v-model="node.data.text" maxlength="20000" class="studio-note-text" :placeholder="t('imageStudio.textPlaceholder')" :aria-label="t('imageStudio.text')" @pointerdown.stop @click.stop @focus="remember" />
              ...
              <template v-else-if="node.type === 'generate'">
                <div class="studio-generator-body"><span class="studio-generator-label">{{ node.data.model || t('imageStudio.generateNode') }}</span><p>{{ generatorPreview(node) || t('imageStudio.generatorHint') }}</p><div class="studio-input-summary"><span>{{ incomingCount(node.id) }} {{ t('imageStudio.source') }}</span><span>{{ node.data.aspectRatio || '1:1' }}</span></div></div>
                <button class="studio-generator-action" @click.stop="openGenerator(node)"><Icon name="play" />{{ t('imageStudio.configure') }}</button>
              </template>
              <template v-else>
        ...
        <aside v-if="activeGenerator" class="studio-inspector" :aria-label="t('imageStudio.configure')" @pointerdown.stop @wheel.stop @keydown.stop @keydown.esc="closeInspector">
          ...
          <form @submit.prevent="generateImage">
            <div class="studio-inspector-scroll">
              ...
              <div v-if="generation.run" class="studio-run"><div class="studio-run-title"><span>{{ t('imageStudio.run') }}</span><strong>{{ runStatus }}</strong></div><p>{{ generation.run.batchId || t('imageStudio.uncertain') }}</p><div v-if="generation.run.job?.actual_cost != null">{{ t('imageStudio.cost') }} <strong>${{ generation.run.job.actual_cost.toFixed(4) }}</strong></div><div class="studio-run-actions"><button v-if="!generation.run.batchId && generation.busy" type="button" :disabled="generation.submitting" @click="generation.retrySubmission()">{{ t('imageStudio.retry') }}</button><button v-if="generation.run.batchId" type="button" :disabled="generation.polling" @click="generation.refresh()">{{ t('imageStudio.refresh') }}</button><button v-if="generation.run.batchId && generation.busy" type="button" :disabled="generation.cancelling" @click="cancelRun">{{ t('imageStudio.cancel') }}</button></div></div>
              <p v-if="generation.error" class="studio-error" role="alert">{{ generation.error }}</p>
              <router-link class="studio-jobs-link" to="/batch-image">{{ t('imageStudio.jobs') }}<Icon name="arrowRight" /></router-link>
            </div>
            <footer><p>{{ t('imageStudio.charge') }}</p><button class="studio-button studio-primary" type="submit" :disabled="!canGenerate"><Icon name="play" />{{ t(generation.busy ? 'imageStudio.pending' : 'imageStudio.generate') }}</button><small>{{ t('imageStudio.waitHint') }}</small></footer>
          </form>
        </aside>
      </div>
...
const runStatus = computed(() => {
  ...
  if (status === 'completed') return t('imageStudio.completed')
  if (status === 'failed' || status === 'output_deleted') return t('imageStudio.failed')
...
watch([activeGenerator, () => generation.models, availableSizes, availableRatios], () => {
  ...
  if (!data || generation.loading) return
...
function addText() { if (!ready.value || !makeRoom()) return; remember(); const node = createStudioNode('text', newPosition(), { text: '' }, t('imageStudio.text')); board.value.nodes.push(node); selectedId.value = node.id; tool.value = 'select'; nextTick(() => canvas.value?.querySelector<HTMLTextAreaElement>(`[data-node-id="${node.id}"] textarea`)?.focus()) }
...
function nodeClick(node: ImageStudioNode) { if (tool.value === 'connect') { if (node.type === 'generate') completeConnection(node.id); else beginConnection(node.id); return } selectedId.value = node.id; selectedEdgeId.value = '' }
function selectEdge(id: string) { selectedEdgeId.value = id; selectedId.value = '' }
...
async function generateImage() {
  ...
  finally { preparingGeneration.value = false }
}
async function addGeneratedImage(blob: Blob, meta: { batchId: string; customId: string; prompt: string; model: string; contextId?: string }) {
  ...
  const node = createStudioNode('result', source ? { x: source.position.x + 400, y: source.position.y } : newPosition(), { images: [asset], prompt: meta.prompt, batchId: meta.batchId, customId: meta.customId, ...(source ? { generateNodeId: source.id } : {}) }, t('imageStudio.result'))
  remember(); board.value.nodes.push(node); selectedId.value = node.id
  await nextTick()
  if (!(await flushSave())) throw new Error(t('imageStudio.saveFailed'))
  if (source && inspectorId.value === source.id) { inspectorId.value = ''; await nextTick(); fit() }
...
async function importBoard(event: Event) {
  ...
  fileBusy.value = true
  try { await flushSave(); const json = await file.text(); ensureActive(); const imported = await storage.importPortableDocument(json, { documentId: 'default' }); ensureActive(); remember(); board.value = imported; reconcileSelection(); await hydrateImages(); fit(); notify(t('imageStudio.imported')) }
  catch (error) { notify(errorText(error)) }
  finally { fileBusy.value = false }
}
async function newBoard() { if (generation.busy || preparingGeneration.value || (board.value.nodes.length && !window.confirm(t('imageStudio.newConfirm')))) return; remember(); board.value = createStudioDocument('default', t('imageStudio.untitled')); reconcileSelection(); await flushSave() }
...
onBeforeUnmount(() => { disposed = true; generation.dispose(); clearTimeout(noticeTimer); clearTimeout(saveTimer); window.removeEventListener('beforeunload', beforeUnload); for (const url of Object.values(urls)) URL.revokeObjectURL(url); void saveQueue.finally(() => { storage.close(); releaseTabLock?.() }) })
</script>
